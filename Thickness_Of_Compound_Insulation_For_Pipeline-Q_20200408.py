...
elif W*D_2 > 0.8: #上一版本，20200405版中错写为W > 0.8，并按此条件计算，这一版更正！！！！！！
...
def GB50264_q_to_T_s(x):
    ...
    return np.array([(x[0]-T_a)*math.pi*x[2]*alpha_s-x[3],
                     (T_1-x[0])/np.log(x[2]/x[1])*2*math.pi*(k2_0 + (T_1/2+x[0]/2)*k2_1 + (T_1/2+x[0]/2)**2*k2_2)-x[3],
                     (T_0-T_1)/np.log(x[1]/D_0)*2*math.pi*(k1_0 + (T_0/2+T_1/2)*k1_1 + (T_0/2+T_1/2)**2*k1_2)-x[3],
                     math.pi*x[2]*Q-x[3]])
...
#传热方程组设立，由D_1、D_2求解 q、T_1、T_s、Q，外表面换热系数的选取方法参照标准《GB50264-2013》
...
sol_root1 = root(GB50264_q_to_T_s,[60,D_0+0.02,D_0+0.04,math.pi*(D_0+0.04)*Q]) 
sol_fsolve1 = fsolve(GB50264_q_to_T_s,[60,D_0+0.02,D_0+0.04,math.pi*(D_0+0.04)*Q])
T_s = sol_fsolve1[0]
D_1 = sol_fsolve1[1]
D_2 = sol_fsolve1[2]
q = sol_fsolve1[3]
...
#print("solution :",sol_root3)
print(1,round(delta_1,2),round(delta_2-0.02,2),round(q,2),round(T_1,2),round(T_s,2),round(Q,2))
...
Q = q/math.pi/D_2
#print("solution :",sol_root3)
print(2,round(delta_1,2),round(delta_2-0.01,2),round(q,2),round(T_1,2),round(T_s,2),round(Q,2))
...
Q = q/math.pi/D_2
#print("solution :",sol_root3)
print(3,round(delta_1,2),round(delta_2,2),round(q,2),round(T_1,2),round(T_s,2),round(Q,2))
...
Q = q/math.pi/D_2
#print("solution :",sol_root3)
print(4,round(delta_1,2),round(delta_2+0.01,2),round(q,2),round(T_1,2),round(T_s,2),round(Q,2))
...
Q = q/math.pi/D_2
#print("solution :",sol_root3)
print(5,round(delta_1,2),round(delta_2+0.02,2),round(q,2),round(T_1,2),round(T_s,2),round(Q,2))
