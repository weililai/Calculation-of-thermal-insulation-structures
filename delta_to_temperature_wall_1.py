...
def GB50264_delta_to_T_s(x):
    Q,T_s = x[0],x[1]
    return np.array([( T_0 -T_s )/(delta/lamb(material,T_0,T_s))-Q,
                     ( T_s -T_a )/(1/alpha_s(T_s,T_a,v,H))-Q
                     ])

#传热方程组求解
sol_root = root(GB50264_delta_to_T_s,[Q_max,T_s_assumption]) 
sol_fsolve = fsolve(GB50264_delta_to_T_s,[Q_max,T_s_assumption])
Q = sol_fsolve[0]
T_s = sol_fsolve[1]
