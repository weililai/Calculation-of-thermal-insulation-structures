...
K = 1.2 #支架修正系数
T_a = 25 #环境温度
Q_assump = 236 #表面热损（W/m2）预估，暂写GB50264-2013附录B要求500摄氏度管道的允许热损（W/m2）
T_s_assump = 42 #表面温度预估
epsilon = 0.75 #外护层材料的黑度，参考GB50264-2013的5.8.9
W = 1.5 #风速
material = 8 #指定保温材料，各数值代表的材料类型参看下面的 def lamb(x,T1,T2) 一段
delta = 0.12 #保温层厚度
...
def GB50264_D_to_T_s(x):
    ...
    return np.array([(T_s-T_a)*math.pi*D_1*alpha_s(T_s,T_a,W,D_1)-q,
                     (T_0-T_s)/np.log(D_1/D_0)*2*math.pi*lamb(material,T_0,T_s)-q
                     ])
