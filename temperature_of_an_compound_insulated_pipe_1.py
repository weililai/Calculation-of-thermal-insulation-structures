...
W = 1 #风速
material = 8 #指定保温层材料，各数值代表的材料类型参看下面的 def lamb(x,T1,T2) 一段
delta = 0.20 #保温层厚度
T_s_assumption = 32.7 #表面温度猜测值


#通过指定材料和热、冷面温度来确定该材料的导热系数，参照标准《GB50264-2013》附录A 常用绝热材料性能，第8、9种材料是CAS产品
def lamb(x,T1,T2) :
...
#线热损、表面温度计算,公式取自标准《GB50264-2013》公式5.4.3
D_1 = D_0 + 2*delta
T_s = T_s_assumption

q = (T_0 - T_a)/(  np.log(D_1/D_0)/(2*lamb(material,T_0,T_s)*math.pi)  +  (1/(math.pi*D_1*alpha_s(T_s,T_a,W,D_1)))  )
T_s = T_a + q*(1/(math.pi*D_1*alpha_s(T_s,T_a,W,D_1)))


print(">>>>>>>>>>> When the delta is ",delta," :")
print("    delta = ",delta)
print("     lamb = ",lamb(material,T_0,T_s))
print("       q  = ",q)
print("      T_s = ",T_s)
