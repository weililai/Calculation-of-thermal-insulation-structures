...
T_a = 20 #环境温度
T_1_max = 400 #第一个界面最高温度
T_2_max = 300 #第二个界面最高温度
Q_max = 236 #允许热损
T_s_max = 39 #最高表面温度
epsilon = 0.25 #外护层材料的黑度
W = 1 #风速
material_1 = 8 #指定内层材料
material_2 = 9 #指定中层材料
material_3 = 1 #指定外层材料
...
#传热方程组设立，由表面温度T_s求解 q、D_1、D_2、D_3，外表面换热系数的选取方法参照标准《GB50264-2013》
def GB50264_T_s_to_q(x):
    q,D_1,D_2,D_3 = x[0],x[1],x[2],x[3]
...
#传热方程组设立，由D_1、D_2求解 q、T_1、T_2、T_s，外表面换热系数的选取方法参照标准《GB50264-2013》
...
T_2=T_2_max
T_s=T_s_max
sol_root2 = root(GB50264_T_s_to_q,[math.pi*(D_0+0.20)*Q,D_0+0.04,D_0+0.10,D_0+0.20]) 
sol_fsolve2 = fsolve(GB50264_T_s_to_q,[math.pi*(D_0+0.20)*Q,D_0+0.04,D_0+0.10,D_0+0.20])
q = sol_fsolve2[0]
D_1 = sol_fsolve2[1]
D_2 = sol_fsolve2[2]
D_3 = sol_fsolve2[3]
...
print(">>>>>>>>>>> When the T_1 T_2 T_s are ",T_1,T_2,T_s," :")
print("  q = ",q)
print("  Q = ",Q)
print("  D_1 = ",D_1)
print("  D_2 = ",D_2)
...
delta = round(D_3/2-D_0/2+0.005,2)
delta_1 = round(D_1/2-D_0/2+0.005,2)
delta_2 = round(D_2/2-D_1/2+0.005,2)
delta_3 = delta-delta_1-delta_2
print(" ")
...
D_2 = D_1 + 2*delta_2
D_3 = D_2 + 2*delta_3
sol_root3 = root(GB50264_D_to_T_s,[q,T_1,T_2,T_s]) 
sol_fsolve3 = fsolve(GB50264_D_to_T_s,[q,T_1,T_2,T_s])
q = sol_fsolve3[0]
T_1 = sol_fsolve3[1]
T_2 = sol_fsolve3[2]
T_s = sol_fsolve3[3]
