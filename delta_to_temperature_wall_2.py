...
delta_2 = 0.12 #外层厚度
Q_max = 236 #允许热损
T_1_assumption = 418 #界面温度猜测值
T_s_assumption = 54 #表面温度猜测值
...
def GB50264_delta_to_T_s(x):
    ...
    return np.array([( T_0 -T_1 )/(delta_1/lamb(material_1,T_0,T_1))-Q,
                     ( T_1 -T_s )/(delta_2/lamb(material_2,T_1,T_s))-Q,
                     ( T_s -T_a )/(1/alpha_s(T_s,T_a,v,H))-Q
                     ])

#传热方程组求解
sol_root = root(GB50264_delta_to_T_s,[Q_max,T_1_assumption,T_s_assumption]) 
sol_fsolve = fsolve(GB50264_delta_to_T_s,[Q_max,T_1_assumption,T_s_assumption])
Q = sol_fsolve[0]
T_1 = sol_fsolve[1]
T_s = sol_fsolve[2]

#打印结果
print(">>>>>>>>>>> When the delta_1 delta_2 is ",delta_1,delta_2," :")
